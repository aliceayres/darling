...
4.7 - Performance on other datasets
Compute the cross entropy loss 计算交叉熵损失函数
numpy.linspace(start, stop, num=50, endpoint=True, retstep=False, dtype=None)
 - 在指定的间隔内返回均匀间隔的数字
 - 返回num均匀分布的样本，在[start, stop]
 - 这个区间的端点可以任意的被排除在外
numpy.c_ & numpy.r_
np.ravel() inplace & np.flatten() copy
'''
...
# GRADED FUNCTION: initialize_parameters (random w, zero b)
...
def compute_cost(A2, Y, parameters):
    ...
    Returns:
    cost -- cross-entropy cost given equation (13) 计算交叉熵损失函数
    """
...
def backward_propagation(parameters, cache, X, Y):
    ...
    dZ1 = np.dot(W2.T, dZ2) * (1 - np.power(A1, 2)) # tanh'(z) = 1 - a^2
...
# GRADED FUNCTION: update_parameters 1 turn
